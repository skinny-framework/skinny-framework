...
class HTTP extends LoggerProvider {
  ...
  def asyncGet(url: String, charset: String = DEFAULT_CHARSET)(implicit ctx: EC): Future[Response] =
    Future(get(url, charset))
  def asyncGet(url: String, queryParams: (String, Any)*)(implicit ctx: EC): Future[Response] =
    Future(get(url, queryParams: _*))
  ...
  def asyncPost(url: String, formParams: (String, Any)*)(implicit ctx: EC): Future[Response] =
    Future(post(url, formParams: _*))
  def asyncPostMultipart(url: String, data: FormData*)(implicit ctx: EC): Future[Response] =
    Future(postMultipart(url, data: _*))
  ...
  def asyncPut(url: String, formParams: (String, Any)*)(implicit ctx: EC): Future[Response] =
    Future(put(url, formParams: _*))
  def asyncPutMultipart(url: String, data: FormData*)(implicit ctx: EC): Future[Response] =
    Future(putMultipart(url, data: _*))
    ...
    try {
      try {
        if (request.bodyBytes.isDefined) {
          ...
          using(conn.getOutputStream) { out =>
            request.requestBody.asBytes.foreach(b => out.write(b))
          }
          ...
          using(conn.getOutputStream) { out =>
            out.write(request.requestBody.asMultipart(boundary))
          }
        }
      ...
      val response: Response = Response(
        ...
        charset = {
          Option(conn.getHeaderField("Content-Type"))
            .map { contentType =>
        ...
        headerFields = conn.getHeaderFields.asScala.map { case (k, vs) => k -> vs.asScala.toSeq }.toMap,
        headers = conn.getHeaderFields.keySet.asScala.map(name => name -> conn.getHeaderField(name)).toMap,
        rawCookies = Option(conn.getHeaderFields.get("Set-Cookie"))
          .map { setCookies =>
