...
addSbtPlugin(\"org.scala-js\" % \"sbt-scalajs\" % \"0.6.4\")" > ${setting_file}
...
if [ "$command" == "new" ]; then
  ...
  # node_modules can be configured with PREFIX env variable
  # homebrew formula will set this value to avoid using universal npm modules
  if [ "${PREFIX}" == "" ]; then
