...
trait SkinnyMailerSmtpConfigApi extends SkinnyMailerConfigBase {
   ...
   */
  def host: String = loadedConfig.flatMap(c => Option(c.getString("smtp.host"))).getOrElse("localhost")

  /**
   * The SMTP server port to connect to, if the connect() method doesn't explicitly specify one. Defaults to 25.
   */
  def port: Int = loadedConfig.flatMap(c => Option(c.getInt("smtp.port"))).getOrElse(587) // default: smtps
   ...
   */
  def connectTimeoutMillis: Int = loadedConfig.flatMap(c => Option(c.getInt("smtp.connectTimeoutMillis"))).getOrElse(3000)

  /**
   * Socket read timeout value in milliseconds. This timeout is implemented by java.net.Socket. Default is infinite timeout.
   */
  def readTimeoutMillis: Int = loadedConfig.flatMap(c => Option(c.getInt("smtp.readTimeoutMillis"))).getOrElse(10000)
   ...
   */
  def startTLSEnabled: Boolean = loadedConfig.flatMap(c => Option(c.getBoolean("smtp.starttls.enabled"))).getOrElse(true)

  /**
   * If true, attempt to authenticate the user using the AUTH command. Defaults to false.
   */
  def authEnabled: Boolean = loadedConfig.flatMap(c => Option(c.getBoolean("smtp.auth.enabled"))).getOrElse(false)

  /**
   * Default user name for SMTP authentication if exists.
   */
  def user: Option[String] = loadedConfig.flatMap(c => Option(c.getString("smtp.auth.user")))

  /**
   * Default passsword for SMTP authentication if exists.
   */
  def password: Option[String] = loadedConfig.flatMap(c => Option(c.getString("smtp.auth.password")))
