...
trait RequestScopeFeature
  ...
  def requestScope(keyAndValues: Seq[(String, Any)])(implicit ctx: SkinnyContext): RequestScopeFeature = {
    keyAndValues.foreach {
      case (key, _) =>
        if (key == "layout") {
          logger.warn(
            "'layout' is a special attribute for Scalate. " +
  ...
  /**
    * Set params to request scope.
    */
  def setParamsToRequestScope(implicit ctx: SkinnyContext = context): Unit =
    set(ATTR_PARAMS -> Params(params(ctx)))(ctx)
  ...
  def addParam(name: String, value: Any)(implicit ctx: SkinnyContext = context): Unit = {
    ...
    val isParamsInRequestScopeValid =
      getFromRequestScope[Any](ATTR_PARAMS)(ctx).map(_.isInstanceOf[Params]).getOrElse(false)

    if (isParamsInRequestScopeValid) {
      val updatedParams: Params = {
        Params(
          getFromRequestScope[Params](ATTR_PARAMS)(ctx)
            .map(_.underlying)
  ...
  def errorMessages(implicit ctx: SkinnyContext = context): Seq[String] = {
    requestScope(ctx)
      .get(RequestScopeFeature.ATTR_ERROR_MESSAGES)
  ...
  def keyAndErrorMessages(implicit ctx: SkinnyContext = context): Map[String, Seq[String]] = {
    requestScope(ctx)
      .get(RequestScopeFeature.ATTR_KEY_AND_ERROR_MESSAGES)
      .map(_.asInstanceOf[KeyAndErrorMessages].underlying)
      .getOrElse(Map())
  }
