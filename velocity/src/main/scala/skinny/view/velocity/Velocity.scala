...
case class Velocity(velocityView: VelocityView) {
  def render(path: String, values: Map[String, Any], request: HttpServletRequest, response: HttpServletResponse): String = {
  ...
  def templateExists(path: String): Boolean = {
    velocityView.getVelocityEngine.resourceExists(path)
  }
}
