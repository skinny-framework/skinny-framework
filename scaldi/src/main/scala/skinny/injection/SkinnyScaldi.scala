...
import scaldi.{ Injector, Module, MutableInjectorAggregation }
...
object SkinnyScaldi extends SkinnyScaldi {
  ...
  /**
    ...
    */
  def modulesForEnv(env: String = SkinnyEnv.getOrElse(SkinnyEnv.Development),
                    defaultModules: Seq[Module] = Nil): MutableInjectorAggregation = {
    scaldiInjectors.getOrElseUpdate(
      env, {
        new MutableInjectorAggregation(
          defaultModules.toList ::: SkinnyConfig
            .stringSeqConfigValue("scaldi.modules")
            .map { moduleClasses: Seq[String] =>
                ...
                try {
                  ...
                  case NonFatal(e) =>
                    throw new ScaldiConfigException(
                      s"Failed to load a Scaldi module because of ${e.getClass.getCanonicalName} (${e.getMessage})",
                      e
                    )
                }
              }.toList
