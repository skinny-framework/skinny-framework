...
class FileUploadTestHelpersTestServlet extends SkinnyEngineServlet with FileUploadSupport with LoggerProvider {
...
class FileUploadTestHelpersTest extends ScalatraFunSuite {
  ...
  val files = Map(
    "textFile" -> new File("engine/src/test/resources/org/scalatra/servlet/lorem_ipsum.txt"),
    "binaryFile" -> new File("engine/src/test/resources/org/scalatra/servlet/smiley.png")
  )
