...
class FileUploadSupportSpec extends MutableScalatraSpec {
  ...
  def postExample[A](f: => A): A = {
    ...
    val files = Map(
      "text" -> new File("scalatra/src/test/resources/org/scalatra/servlet/lorem_ipsum.txt"),
      "binary" -> new File("scalatra/src/test/resources/org/scalatra/servlet/smiley.png")
    )
  ...
  def postMultiExample[A](f: => A): A = {
    val files =
      ("files[]", new File("scalatra/src/test/resources/org/scalatra/servlet/lorem_ipsum.txt")) ::
        ("files[]", new File("scalatra/src/test/resources/org/scalatra/servlet/smiley.png")) :: Nil
  ...
  def postPass[A](f: => A): A = {
    val params = Map("param1" -> "one", "param2" -> "two")
    val files = Map("text" -> new File("scalatra/src/test/resources/org/scalatra/servlet/lorem_ipsum.txt"))
  ...
  "POST with multipart/form-data and maxFileSize set" should {
    "handle IllegalStateException by wrapping it as SizeConstraintExceededException handled by error handler" in {
      post("/max-size/upload", Map(), Map("file" -> new File("scalatra/src/test/resources/org/scalatra/servlet/smiley.png"))) {
    ...
    "allow file uploads smaller than the specified max file size" in {
      post("/max-size/upload", Map(), Map("file" -> new File("scalatra/src/test/resources/org/scalatra/servlet/lorem_ipsum.txt"))) {
        body must_== "ok"
      }
    }
  }
  ...
  "FileItem.write" should {
    "know how to write to a File instance" in {
      val params = Map()
      val files = Map("document" -> new File("scalatra/src/test/resources/org/scalatra/servlet/lorem_ipsum.txt"))
