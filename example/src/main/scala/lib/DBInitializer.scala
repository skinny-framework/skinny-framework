  ...
  addSeed {
    DB localTx { implicit s =>
      val company1 = sql"insert into company (name, url, created_at) values (?, ?, current_timestamp)"
        .bind("Typesafe", "http://typesafe.com/")
        .updateAndReturnGeneratedKey
        .apply()
      val company2 = sql"insert into company (name, url, created_at) values (?, ?, current_timestamp)"
        .bind("Oracle", "http://twww.oracle.com/")
        .updateAndReturnGeneratedKey
        .apply()

      sql"insert into company (name, url, created_at) values (?, ?, current_timestamp)"
        .batch(
      ...
      sql"insert into skill (name) values (?)"
        .batch(
          Seq("Ruby"),
          Seq("MySQL"),
          Seq("PostgreSQL")
      ...
      val insertProgrammer =
        sql"insert into programmer (name, favorite_number, company_id, created_timestamp) values (?, ?, ?, current_timestamp)"
      ...
      sql"insert into programmer_skill (programmer_id, skill_id) values (?, ?)"
        .batch(
