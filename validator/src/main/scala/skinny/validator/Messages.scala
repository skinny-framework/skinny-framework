...
object Messages {
  ...
  private[this] val loadedFromProperties =
    new scala.collection.concurrent.TrieMap[(String, Option[Locale]), Messages]()
  ...
  def loadFromConfig(prefix: String = "messages", locale: Option[Locale] = None): Messages = {
    loadedFromConfig.getOrElseUpdate(
      (prefix, locale), {
        val ext = ".conf"
        val file = locale
          .map { l =>
        ...
        val map: Map[String, String] =
          config.getConfig("error").root().unwrapped().asScala.map { case (k, v) => k -> v.toString }.toMap
        new Messages(map)
      }
    )
  }
  ...
  def loadFromProperties(prefix: String = "messages", locale: Option[Locale] = None) = {
    loadedFromProperties.getOrElseUpdate(
      (prefix, locale), {
        val ext = ".properties"
        val file = locale
          .map { l =>
        ...
        val map: Map[String, String] = new java.util.HashMap[Any, Any](properties).asScala
          .filter {
...
class Messages(map: Map[String, String]) {
  ...
  def get(key: String, params: Seq[Any]): Option[String] = {
    ...
    get(key).map(template => MessageFormat.format(template, _params.map(_.asInstanceOf[Object]): _*))
  }
