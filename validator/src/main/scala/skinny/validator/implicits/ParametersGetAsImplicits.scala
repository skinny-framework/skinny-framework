...
trait ParametersGetAsImplicits {
  ...
  implicit def skinnyValidatorSafe[S, T](f: S => T): ParamValueTypeConverter[S, T] =
    new ParamValueTypeConverter[S, T] {
  ...
  def skinnyValidatorStringToSeq[T: Manifest](elementConverter: ParamValueTypeConverter[String, T],
                                              separator: String = ","): ParamValueTypeConverter[String, Seq[T]] = {
    skinnyValidatorSafe(s => s.split(separator).toSeq.flatMap(e => elementConverter(e.trim)))
  }

  implicit def skinnyValidator_defaultStringToSeq[T](implicit elementConverter: ParamValueTypeConverter[String, T],
                                                     mf: Manifest[T]): ParamValueTypeConverter[String, Seq[T]] = {
    skinnyValidatorStringToSeq[T](elementConverter)
  }

  implicit def skinnyValidatorSeqHead[T](implicit elementConverter: ParamValueTypeConverter[String, T],
                                         mf: Manifest[T]): ParamValueTypeConverter[Seq[String], T] = {
    skinnyValidatorSafeOption(_.headOption.flatMap(elementConverter(_)))
  }

  implicit def skinnyValidatorSeqToSeq[T](implicit elementConverter: ParamValueTypeConverter[String, T],
                                          mf: Manifest[T]): ParamValueTypeConverter[Seq[String], Seq[T]] = {
