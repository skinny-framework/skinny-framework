...
class ValidatorSpec extends AnyFlatSpec with Matchers with ParametersGetAsImplicits {
  ...
  it should "pass all the valid values" in {
    ...
    val res1: Int = validator
      .success { inputs =>
        inputs.getAs[Int]("id").getOrElse(-1)
      }
